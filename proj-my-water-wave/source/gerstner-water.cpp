...
int main(int argc, char* argv[])
{
	...
	GLFWwindow* window = glfwCreateWindow (800, 600, "Gerstner waves", NULL, NULL);
	if (!window) {
...
{
	FILE *f;
	/* NOTICE: 原来是fopen_s 为了使用mingw修改成 fopen */
	fopen(filename, "r");
	void *buffer;
...
static void installShaders(void)
{
	names.vertex_shader = initShader(GL_VERTEX_SHADER, vs_filename.c_str());
	names.fragment_shader = initShader(GL_FRAGMENT_SHADER, fs_filename.c_str());
